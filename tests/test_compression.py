...
class TestCompressedStore:
    ...
    def test_backwards_compat_uncompressed_store(self, temp_db, sample_matrix):
        """Old store without config DBI works (uncompressed)."""
        store = MatrixStore(temp_db)
        store.store_initial("test", sample_matrix)  # no compression

        # Reopen and verify
        store2 = MatrixStore(temp_db)
        reconstructed = store2.get_matrix("test", 1)
        np.testing.assert_array_equal(reconstructed.matrix, sample_matrix.matrix)

        valid, errors = store2.verify_dataset("test")
        assert valid is True
    ...
    def test_config_persisted_in_lmdb(self, temp_db, sample_matrix):
        ...
        with store2.env.begin() as txn:
            config = store2._get_store_config(txn, 'compression')
        assert config == "zlib"
